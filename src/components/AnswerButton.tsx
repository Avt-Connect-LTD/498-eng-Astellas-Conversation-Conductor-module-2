...
const AnswerButton: React.FC<AnswerButtonProps> = ({ answer, isMobile, isSelected, pointsConfig, isSubmitted, isDisabled, onClick, animate, animationDirection, answerButtonHoverState }) => {
  ...
  const base = ` ${isMobile ? "w-[90%]" : "w-full"} font-bold bg-[var(--color-answer-background)]/50 hover:bg-[var(--color-answer-background)]/90 backdrop-blur-lg cursor-pointer text-left pr-8 pl-8 pt-4 pb-4 rounded-lg border border-2 text-xl shadow-md focus:outline-none transform transition-all duration-500 ease-in-out`;
  ...
  return (
    <button onClick={onClick} disabled={isDisabled} className={`${base} ${animation}${stateClasses}${hoverAddon}${disabledAddon}${interactionAddon} text-[var(--color-answer-text)]`}>
      {answer.text}
      {answer.subtext && <p className="italic text-xl font-normal text-white">{answer.subtext}</p>}
    </button>
  );
};
