...
const QuestionBubble: React.FC<QuestionBubbleProps> = ({ person, text, animate }) => {
  return (
    <div className="relative">
      <div
        className={`relative bg-[var(--color-question-background)] backdrop-blur-sm p-4 sm:p-10 rounded-xl shadow-xl text-[var(--color-question-text leading-relaxed w-4xl transition-all duration-500 ${animate ? "translate-y-0 opacity-100" : "translate-y-16 opacity-0"}`}
        style={{
      ...
      >
        <span className="text-2xl font-bold text-[#D01D82]">{person}</span>
          <span className="text-2xl text-[#D01D82]">{text}</span>
        {/* Speech bubble tail */}
        <div 
